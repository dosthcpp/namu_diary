...
class FeedCard extends StatefulWidget {
  final String user, content, feedPath, feedNo, imagePath;
  final DateTime date;
  final url;
...
class _FeedCardState extends State<FeedCard> {
  ...
  delete(feeds, context) async {
    final docRef = feeds.data.docs;
    final deleteId = docRef
        .where((el) => el.data()['feedNo'] == widget.feedNo)
        .elementAt(0)
        .id;
    final imagePath =
        (await _fireStore.collection(widget.feedPath).doc(deleteId).get())
            .data()['imagePath'];
    final fileRef = FirebaseStorage.instance.ref().child(imagePath);
    if (fileRef != null) {
      fileRef.delete();
    }
    await _fireStore.collection(widget.feedPath).doc(deleteId).delete();
    for (var i = int.tryParse(widget.feedNo) ?? 0; i < docRef.length - 1; ++i) {
      final id = docRef
          .where((el) => el.data()['feedNo'] == numberPad((i + 1).toString()))
          .elementAt(0)
          .id;
      // change feedNo
      await _fireStore.collection(widget.feedPath).doc(id).update({
        'feedNo': numberPad(i.toString()),
      });
    }
    int popCount = 0;
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: StreamBuilder<QuerySnapshot>(
        stream: _fireStore
            .collection(widget.feedPath)
            .where('feedNo', isEqualTo: widget.feedNo)
            .limit(1)
            .snapshots(),
        builder: (context, feed) {
          final _feed = feed?.data?.docs?.elementAt(0)?.data() ?? {};
          final likeList = List.from(_feed['like'] ?? []);
          final temp = likeList;
          var like = false;
          var i = 0;
          for (;
                            ...
                            children: [
                              StreamBuilder<QuerySnapshot>(
                                  stream: _fireStore
                                      ...
                                      .where('content',
                                          isEqualTo:
                                              '${(int.tryParse(widget.feedNo) + 1) ?? 1}번째 피드')
                                      .limit(1)
                                      .snapshots(),
                                                                          ...
                                                                          actions: [
                                                                            TextButton(
                                                                              ...
                                                                              onPressed: () async {
                                                                                ...
                                                                                final info = await _fireStore.collection(widget.feedPath).where('feedNo', isEqualTo: widget.feedNo).limit(1).get();
                                                                                await _fireStore.collection(widget.feedPath).doc(info.docs[0].id).update(
                                                                                  {
                                                                                    'reply': reply,
                                                                                  },
                                                                                );
                                                    ...
                                                    if (replyMode) {
                                                      if (replyingUser !=
                                                        ...
                                                        await notify(
                                                          ...
                                                          'reply',
                                                          DateTime.now(),
                                                          "${int.tryParse(widget.feedNo) + 1 ?? 1}번째 피드의 댓글 $replyingContent에 ${replyEditingController.text}(이)라고 남겼습니다!",
                                                          path: widget.feedPath,
                                                          index: widget.feedNo,
                                                        );
                                                      }
                                                      ...
                                                      if (widget.user !=
                                                        ...
                                                        await notify(
                                                          ...
                                                          'reply',
                                                          DateTime.now(),
                                                          "${int.tryParse(widget.feedNo) + 1 ?? 1}번째 피드에 ${replyEditingController.text}(이)라고 남겼습니다!",
                                                          path: widget.feedPath,
                                                          index: widget.feedNo,
                                                        );
                                                      }
